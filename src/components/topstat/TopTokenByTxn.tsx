...
import { TopTokenChart } from "./TopTokenChart";
...
export const TopTokenByTxn = () => {
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => row.token, {
      id: "token",
      header: () => (
        <HeaderTable textStyle=" dark:text-gray200" text={t("topStatPage.tokenTab.address")} />
      ),
      cell: ({ getValue }) => (
        <CellText
          text={
    ...
    columnHelper.accessor((row) => row.txn, {
      id: "txn",
      header: () => (
        <HeaderTable text={t("topStatPage.tokenTab.txnCount")} textStyle="dark:text-gray200" />
      ),
      cell: ({ getValue }) => (
        <CellText
          text={
      ...
      <div className="w-full bg-[#ffff] dark:bg-[#111] rounded-xl border border-[#DCDCDC] overflow-hidden">
        <div className="flex justify-between items-center border-b border-[#e9ecef bg-[#F8F9FA] dark:bg-gray800  p-4 ">
          <div className=" whitespace-nowrap">
            <span>{t("topStatPage.tokenTab.topTokensByTxnCount")}</span>
          </div>
        </div>
        <div className="p-4 grid grid-cols-1 lg:grid-cols-2 items-center gap-5">
          <div><TopTokenChart/></div>
          <DataTable
            data={defaultData}
