...
export const TopAccountByTxn = () => {
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => row.txn, {
      id: "txn",
      header: () => (
        <HeaderTable text={t("topStatPage.networkTab.txnCount")} textStyle="dark:text-gray200" />
      ),
      cell: ({ getValue }) => (
        <CellText
          text={
    ...
    <div>
      <div className="w-full bg-[#ffff] dark:bg-[#111] rounded-xl border border-[#DCDCDC] overflow-hidden">
        <div className="flex justify-between items-center border-b border-[#e9ecef bg-[#F8F9FA] dark:bg-gray800  p-4 ">
          ...
          <span>{t("topStatPage.networkTab.topAccountsByTxnCount")}</span>
          </div>
        </div>
